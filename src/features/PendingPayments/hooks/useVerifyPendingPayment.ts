  ...
  return useMutation({
    ...
    onSuccess: (response, paymentId) => {
      if (response.succeeded) {
        ...
        toast.error(response.message || 'عذراً، هناك مشكلة في تأكيد الدفعة المعلقة الخاصة بالكفالات');
      }
    },

    onError: (error: AxiosError<{ message: string }>) => {
      const message = error?.response?.data?.message || error.message || 'حدث خطأ غير متوقع';
      toast.error(`عذراً، هناك مشكلة في تأكيد الدفعة المعلقة الخاصة بالكفالات: ${message}`);
    },
  });
};