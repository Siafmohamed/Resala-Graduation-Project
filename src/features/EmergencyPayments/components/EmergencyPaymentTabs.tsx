...
interface EmergencyPaymentTabsProps {
  activeTab: EmergencyPaymentMethod;
  onTabChange: (tab: EmergencyPaymentMethod) => void;
  counts?: Record<EmergencyPaymentMethod, number>;
}

const tabs: { id: EmergencyPaymentMethod; label: string; icon: React.ElementType; color: string }[] = [
  { id: 'All', label: 'الكل', icon: LayoutGrid, color: 'blue' },
  { id: 'Representative', label: 'مندوبين', icon: User, color: 'green' },
...
const EmergencyPaymentTabs: React.FC<EmergencyPaymentTabsProps> = ({ activeTab, onTabChange, counts }) => {
  return (
...
export default EmergencyPaymentTabs;
